...
class ColorBarWidget : public QFrame
{
	Q_OBJECT
public:
	ColorBarWidget( QWidget *parent = 0 ) : QFrame( parent ) {
		setMinimumSize( QSize( BARWIDTH + 2, WHEELSIZE + SELECTORSIZE ) );
		barRect = QRect( 0, CURSORMID, BARWIDTH, height() - CURSORMID * 2 );
		setValue( 0 );
		cursor = QPixmap(":/images/icons/cursor.png");
		...
		gradient.setColorAt( 1.0/6.0, QColor(255,255,0) );
		gradient.setColorAt( 2.0/6.0, QColor(0,255,0) );
		gradient.setColorAt( 3.0/6.0, QColor(0,255,255) );
		gradient.setColorAt( 4.0/6.0, QColor(0,0,255) );
		gradient.setColorAt( 5.0/6.0, QColor(255,0,255) );
...
public slots:
	...
	void setValue( double val ) {
		value = qMax( qMin( 1.0, val ), 0.0 );
		gradient.setStart(0, val);
		gradient.setFinalStop(0, val + 1.0);
		barBrush = QBrush( gradient );
		update();
	}
